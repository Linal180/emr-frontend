import { FindAllVaccineProductsPayload } from "../generated/graphql";
...
export interface State {
  ...
  itemId: string;
  isOpen: boolean;
  data: FindAllVaccineProductsPayload['vaccineProducts']
  totalPages: number;
  openDelete: boolean;
  searchQuery: string;
  systematic: boolean;
...
export type Action =
  ...
  { type: ActionType.SET_DATA; data: FindAllVaccineProductsPayload['vaccineProducts'] }

export const vaccineProductReducer = (state: State, action: Action): State => {
  switch (action.type) {
