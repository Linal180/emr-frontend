// packages block
import { FC } from "react";
import { Edit, Save, ArrowBack } from "@material-ui/icons";
import { Card, CardContent, CardHeader, IconButton, Box, Grid } from "@material-ui/core";
// interfaces/types block
import { CardComponentType } from "../../interfacesTypes";

const CardComponent: FC<CardComponentType> = ({
  children, cardTitle, isEdit, hasEdit, onEditClick, disableEditIcon, disableSaveIcon, hideSaveIcon, isFullHeight, saveBtn, saveBtnId, saveBtnRef, activeBar
}): JSX.Element => {

  const SaveHandler = () => {
    activeBar && activeBar()
    saveBtnRef?.current?.scrollIntoView({ behavior: 'smooth' })
  }

  return (
    <Card className={isFullHeight ? "fullMinHeight card-box-shadow" : " card-box-shadow"}>
      <CardHeader
        action={
          hasEdit ? (
            <Box display="flex" alignItems="center">
              {isEdit ? (
                <Box>
                  {!hideSaveIcon && (
                    <IconButton disabled={disableSaveIcon} type="submit" color="primary" aria-label="settings">
                      <Save />
                    </IconButton>
                  )}

                  <IconButton onClick={onEditClick} aria-label="settings">
                    <ArrowBack />
                  </IconButton>
                </Box>
              ) : (
                <IconButton disabled={disableEditIcon} onClick={onEditClick} aria-label="settings">
                  <Edit />
                </IconButton>
              )}
            </Box>
          ) : saveBtn && (
            <Grid id={saveBtnId} ref={saveBtnRef}>
              <IconButton onClick={() => SaveHandler()} color="primary" aria-label="settings">
                <Save />
              </IconButton>
            </Grid>

          )
        }
        title={cardTitle}
      />

      <CardContent>{children}</CardContent>
    </Card>
  )
};

export default CardComponent;
