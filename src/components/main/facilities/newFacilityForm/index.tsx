...
import {
  Box, Button, Checkbox, CircularProgress, Collapse, FormControl, FormControlLabel, FormGroup, Grid, List, ListItem, Typography
...
import {
  ...
  CLIA_ID_NUMBER_INFO, TAXONOMY_CODE_INFO, NPI_INFO, MAMOGRAPHY_CERTIFICATION_NUMBER_INFO, FEDERAL_TAX_ID_INFO, FACILITY_INFO_ROUTE, FACILITY_LOCATION_ROUTE, BILLING_PROFILE_ROUTE, FACILITY_SCHEDULE_ROUTE, FACILITY_SCHEDULE, FacilityMenuNav,
} from "../../../../constants";
        ...
        <Box mr={2} ml={2} pl={2} pr={2} pb={4} display='flex' className={classes.navbar}>
          <List>
            {FacilityMenuNav.map((item, index) => {
              console.log("item link", item.linkTo)
              return (
                <a href={`#${item.linkTo}`} className={`#${item.linkTo}` === path ? 'active' : ''}>
                  <Box display='flex'>
                    ...
                    <ListItem
                      button
                      className={`#${item.linkTo}` === path ? 'active' : ''}
