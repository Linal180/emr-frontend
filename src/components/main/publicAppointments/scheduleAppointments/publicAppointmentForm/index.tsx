...
import {
  ...
  useGetTokenLazyQuery, useChargeAfterAppointmentMutation, FacilityPayload, BillingStatus
...
import {
  ...
  SELECT_PROVIDER, BOOK_APPOINTMENT, AVAILABLE_SLOTS, SELECT_DATE, PATIENT_LAST_NAME,
  RELATIONSHIP_WITH_PATIENT, YOUR_NAME, PROVIDER, FACILITY_NOT_FOUND, PATIENT_APPOINTMENT_FAIL,
  APPOINTMENT_SLOT_ERROR_MESSAGE, NO_SLOT_AVAILABLE, BOOK_YOUR_APPOINTMENT, AGREEMENT_HEADING,
  agreementPoints, AGREEMENT_TEXT, SLOT_CONFIRMATION, SEX_AT_BIRTH, CANCEL, PATIENT_FIRST_NAME,
} from "../../../../../constants";
...
const PublicAppointmentForm = (): JSX.Element => {
  ...
  const {
    facility, availableSlots, currentDate, offset, agreed, appointmentPaymentToken, externalAppointment
  } = state;
  ...
  const [chargePaymentMutation, { loading: appointmentLoader }] = useChargeAfterAppointmentMutation({
            ...
            <Grid container spacing={3}>
              {appointmentPaymentToken === '' ?
                <>
                  ...
                  <Grid item md={12}>
                    <Box pt={4} display="flex" justifyContent="center" gridGap={20}>
                      <Link to={PATIENT_APPOINTMENT_FAIL}>
                ...
                :
                getTokenLoading || appointmentLoader ? 'loading....' :
                  <Grid item lg={9} md={8} sm={6} xs={12}>
                    <PaymentForm clientToken={appointmentPaymentToken}
                      amount={externalAppointment?.price}
