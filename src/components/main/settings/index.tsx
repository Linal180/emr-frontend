...
import {
  APPOINTMENT_SETTINGS, APPOINTMENT_SETTINGS_ITEMS, CALENDAR_SETTINGS_ITEMS, CALENDAR_SETTINGS_TEXT, CLINICALS_ITEMS, CLINICALS_TEXT, INVENTORY, INVENTORY_ITEMS, MISCELLANEOUS_SETTINGS,
    ...
    <Grid container justifyContent='center'>
      <Grid item md={7} sm={12} xs={12}>
        ...
        <CardComponent cardTitle={CALENDAR_SETTINGS_TEXT}>
          <Box pb={3}>
            {CALENDAR_SETTINGS_ITEMS.map((item) => {
              return (
                <Box display="flex" alignItems="center" flexWrap="wrap">
                  <Link key={`${item.link}-${item.name}`} to={item.link}>
                    <MenuItem>{item.name}</MenuItem>
        ...
        <CardComponent cardTitle={CLINICALS_TEXT}>
          <Box pb={3}>
            {CLINICALS_ITEMS.map((item) => {
              return (
                <Box display="flex" alignItems="center" flexWrap="wrap">
                  <Link key={`${item.link}-${item.name}`} to={item.link}>
                    <MenuItem>{item.name}</MenuItem>
