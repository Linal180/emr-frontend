...
import {
  ACTION, COLUMN_LENGTH, CSS_CLASSES, FIELD_EDIT_INITIAL_VALUES, LABEL, NAME, NO_TEXT, OPTION_TEXT, PLACEHOLDER, 
  PROPERTIES_TEXT, REQUIRED_TEXT, SELECT_COLUMN_TEXT, VALUE, YES_TEXT, SAVE_TEXT, REGEX_LABEL, FUTURE_DATE, PAST_DATE
} from '../../../../../constants';
//styles
...
const FieldProperties = ({ setFieldValuesHandler, selected }: FieldEditModalProps) => {
  ...
  const setFormInitialValues = useCallback(() => {
    const { 
      name, label, required, column, placeholder, css, fieldId, type, list, errorMsg, defaultValue, options, textArea, 
      regex, pastEnable, futureEnable 
    } = selected;
    setValue("name", name)
  ...
  return (
    <FormProvider {...methods}>
      <Box mt={2} pb={2} borderBottom={`1px solid ${colors.grey[300]}`} display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap">
